...
- **`LightClientStore` is the single source of truth** for sync committee
  data (`current_sync_committee`, `next_sync_committee`) and period
  (derived from `finalized_header.slot`).  `sync_committee.rs` provides
  stateless free functions that accept committee references and a
  store-derived period.  There is no separate tracker or period counter.
...
- Separate period counters or tracker structs for committee state
...
5. Run `./scripts/warner-check.sh` (fmt + test + clippy)
...
1. Read `src/consensus/sync_committee.rs` —
   `compute_sync_committee_domain_for_slot()`, `verify_sync_aggregate()`
...
3. Do not expose `LightClientStore` or `LightClientProcessor`
