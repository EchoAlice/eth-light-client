...
mod tests {
    ...
    fn test_timestamp_to_slot_minimal() {
        let spec = ChainSpec::minimal();
        assert_eq!(spec.timestamp_to_slot(1578009600), 0);
        assert_eq!(spec.timestamp_to_slot(1578009600 + 6), 1);
        assert_eq!(spec.timestamp_to_slot(1578009600 + 60), 10);
        assert_eq!(spec.timestamp_to_slot(1578009600 - 100), 0);
    }
    ...
    fn test_fork_at_epoch_mainnet() {
        ...
        assert_eq!(spec.fork_at_epoch(74240), Fork::Altair);
    ...
    fn test_fork_version_at_epoch_mainnet() {
        let spec = ChainSpec::mainnet();
        assert_eq!(spec.fork_version_at_epoch(74240), [0x01, 0x00, 0x00, 0x00]);
        assert_eq!(spec.fork_version_at_epoch(144896), [0x02, 0x00, 0x00, 0x00]);
        assert_eq!(spec.fork_version_at_epoch(194048), [0x03, 0x00, 0x00, 0x00]);
        assert_eq!(spec.fork_version_at_epoch(269568), [0x04, 0x00, 0x00, 0x00]);
    ...
    fn test_gindex_altair_through_deneb() {
        let spec = ChainSpec::mainnet();
        let altair_slot = 74240 * 32;
        let deneb_slot = 269568 * 32;
