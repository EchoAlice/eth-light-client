...
/// TODO(#21): make committee size dynamic via ChainSpec instead of hardcoded 512
...
impl SyncCommittee {
    /// TODO(#21): replace with dynamic size from ChainSpec
    ...
    /// Handles both minimal preset (32 keys) and mainnet (512 keys).
...
/// TODO(#21): make bits array spec-sized
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SyncAggregate {
    /// Participation bits for 512 sync committee members (heap-allocated)
...
///
/// Contains a trusted beacon block header, the sync committee at that slot,
/// a merkle proof, and the genesis validators root.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn create_test_beacon_header() -> BeaconBlockHeader {
        BeaconBlockHeader::new(1000, 42, [1u8; 32], [2u8; 32], [3u8; 32])
    }
    ...
    fn test_sync_committee_supermajority() {
        ...
        for bit in participation.iter_mut().take(341) {
            *bit = true;
        }
        ...
        let mut participation = [false; 512];
        for bit in participation.iter_mut().take(340) {
            *bit = true;
        }
        assert!(!committee.has_supermajority_participation(&participation));
    ...
    fn test_light_client_update_validation() {
        ...
        let update = LightClientUpdate::new(attested_header, sync_aggregate, 1001);
